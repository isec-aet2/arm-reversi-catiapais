...
volatile uint8_t flagPrintMenu=0;
volatile uint8_t counter = 0;
volatile uint8_t counterPlay = 20;
long int JTemp = 0;
char string[100];
char rules[1000];
...
void ReadRulesFromSD ()
{
	...
	if(f_open (&SDFile, "Rules.txt", FA_READ)!=FR_OK) //tenta criar ficheiro em modo escrita e testa se detectou SD
		Error_Handler();
...
void writeGameInfoSD ()
{
	...
	if(f_open (&SDFile, "logtemp.txt", FA_WRITE | FA_CREATE_ALWAYS)!=FR_OK) //tenta criar ficheiro em modo escrita e testa se detectou SD
		Error_Handler();
	sprintf(string," %ld ", JTemp);
	if(f_write (&SDFile, string, strlen(string), &nBytes)!=FR_OK) // strlen(string) ->numero de caracteres sizeof->n de bytes
		Error_Handler();
	f_close (&SDFile);
}
